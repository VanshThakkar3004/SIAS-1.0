...
# -------------------- PAGE CONFIG -------------------- #
st.set_page_config(page_title="🛰️ Satellite Image & Soil Analysis", layout="wide", page_icon="🌍")
...
.stTabs [role="tab"]:hover { background-color: #dbeafe; }
.stButton>button { background-color: #0066cc; color: white; border-radius: 5px; padding: 8px 16px; }
.stDownloadButton>button { background-color: #10b981; color: white; border-radius: 5px; }
</style>
""", unsafe_allow_html=True)

st.title("🛰️ Satellite Image & Soil Analysis")
...
# -------------------- TABS -------------------- #
tab1, tab2, tab3 = st.tabs(["📸 Image Analyzer", "🗺️ Interactive Map", "🌾 Soil & Crop Analysis"])
...
with tab1:
    ...
    if uploaded_file:
        ...
        # -------------------- CNN CLASSIFICATION -------------------- #
        st.subheader("🧠 CNN Classification (Deforestation Detection)")
        ...
        if model_outputs:
            ...
            with open(pdf_output_path, "rb") as file:
                st.download_button("📄 Download PDF Report", file, "satellite_classification_report.pdf", mime="application/pdf")
...
with tab2:
    ...
    with col_map:
        st.markdown("Click anywhere on the map to get coordinates:")
...
with tab3:
    ...
    if soil_file:
        ...
        if not os.path.exists(model_path):
            st.error("❌ Soil classification model not found. Please train and place 'soil_model.pth' in the app directory.")
        else:
            # Define device
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            ...
            # Load the saved model state dict to inspect its shape
            ...
            # Soil class mapping (you can adjust names as per your dataset)
...
# -------------------- FOOTER -------------------- #
st.markdown("---")
st.markdown("<center>© 2025 Satellite Analysis Platform | Powered by MobileNetV2 & EfficientNetB0</center>", unsafe_allow_html=True)
